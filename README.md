...
Automated extraction of tables from clinical PDF documents, with confidence-based routing to avoid sending difficult tables to a model that will produce bad output.
...
| Signal | ROC-AUC |
|--------|---------|
| Structure decoder confidence | 0.57 |
| Trained binary classifier (best attempt) | 0.58-0.66 |

The confidence score matches or beats a trained classifier with zero additional training. At the 100% precision threshold (`-0.00001`), no bad tables pass through. Recall at that point is ~2%, so the practical operating point trades some precision for more coverage. See `notebooks/research/04_confidence_threshold_analysis.ipynb` for the full sweep.
...
Start with `notebooks/research/` in numerical order. Each notebook has a short header explaining what it tests and what conclusion it reached.

For the final pipeline see `agents/unitable_agent.py` and `notebooks/pipeline.ipynb`.
